...
function Nav() {
  ...
  // 1. Create a ref to track the profile dropdown container
  const profileRef = useRef(null);

  // 2. Add an effect to listen for clicks outside of the ref
  useEffect(() => {
  ...
  return (
    ...
    <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] z-10'>
     ...
     <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
        ...
        {!userData ? (
          <IoMdPerson className='w-[50px] h-[50px] fill-white cursor-pointer border-[2px] border-[#fdfbfb] bg-[#000000d5] rounded-full p-[10px]' onClick={()=>navigate("/login")}/>
        ) : (
          <div
            className='relative'
            ref={profileRef} 
          >
            ...
            {showPro && (
              <div className='absolute top-[110%] right-0 flex items-center flex-col justify-center gap-2 text-[16px] rounded-md bg-[white] px-[15px] py-[10px] border-[2px] border-black min-w-[180px] shadow-lg'>
                <span className='bg-[black] text-white px-[30px] py-[10px] rounded-2xl hover:bg-gray-600 cursor-pointer w-full text-center' onClick={()=>{navigate("/profile"); setShowPro(false);}}>My Profile</span>
              </div>
            )}
          </div>
        )}
