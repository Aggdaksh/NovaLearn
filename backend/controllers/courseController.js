...
export const getPublishedCourses = async (req, res) => {
    try {
        let courses = await Course.find({ isPublished: true }).populate("lectures");
        const usingDraftFallback = courses.length === 0;

        if (usingDraftFallback) {
            courses = await Course.find({}).populate("lectures");
        }
